...
int Merge::Process(int16_t* input, size_t input_length,
  ...
  for (size_t channel = 0; channel < num_channels_; ++channel) {
    ...
    *external_mute_factor =
    ...
    static unsigned int kTempDataSize = 8192;
    unsigned int decoded_len = best_correlation_index + input_length_per_channel;
    unsigned int decoded_output_max_len = std::max(decoded_len, kTempDataSize);
    rtc::scoped_ptr<int16_t[]> temp_data(new int16_t[decoded_output_max_len]);  // TODO(hlundin) Remove this.
    int16_t* decoded_output = temp_data.get() + best_correlation_index;
    ...
    memmove(temp_data.get(), expanded_channel,
    ...
    memcpy(&(*output)[channel][0], temp_data.get(),
