...
Operations DecisionLogicNormal::ExpectedPacketAvailable(Modes prev_mode,
  ...
  if (prev_mode != kModeExpand && !play_dtmf) {
    ...
    s_log_count++;
    if (0 == s_log_count % 100) {
      int packet_len_ms = delay_manager_->GetPacketAudioLenght();
      AVENGINE_TRC("audio jitter: low_ms=%d, hight_ms=%d, ave_buffer_ms=%d",
        low_limit * packet_len_ms / 256, high_limit * packet_len_ms / 256,
        buffer_level_filter_->filtered_current_level() * packet_len_ms / 256);
    }

    if (buffer_level_filter_->filtered_current_level() >= high_limit) {
      if (TimescaleAllowed() || buffer_level_filter_->filtered_current_level() >= (high_limit << 2)) {
        // Buffer level higher than limit and time-scaling allowed,
        // or buffer level really high.
        return kAccelerate;
      }
      int packet_len_ms = delay_manager_->GetPacketAudioLenght();
      if (packet_len_ms > 0 && ((buffer_level_filter_->filtered_current_level() - high_limit) * packet_len_ms / 256 > 150)) {
        return kAccelerate;
      }
