...
void D3D9Channel::SetUserdata(void* data) {
  _userdata = data;
}

void* D3D9Channel::GetUserdata() {
  return _userdata;
}
...
int D3D9Channel::IsUpdated(bool& isUpdated, bool& firstUpdated) {
  CriticalSectionScoped cs(_critSect);
  isUpdated = _bufferIsUpdated;
  firstUpdated = _bufferIsUpdated && _firstUpdated;
  return 0;
}
...
int D3D9Channel::ReleaseTexture()
{
    ...
    _pd3dDevice = NULL;
    AVENGINE_INF("D3D9Channel::ReleaseTexture");
    return 0;
}
...
int VideoRenderDirect3D9::UpdateRenderSurface()
{
    ...
    std::set<D3D9Channel*> first_channels;

    // Check if there are any updated buffers
    bool updated = false;
    for (auto it = _d3dChannels.begin(); it != _d3dChannels.end(); it++) {
      D3D9Channel* channel = it->second;
      bool channel_updated = false;
      bool first_updated = false;
      channel->IsUpdated(channel_updated, first_updated);

      if (channel_updated) {
        updated = true;
      }
      if (first_updated) {
        first_channels.insert(channel);
      }
    }
...
bool VideoRenderDirect3D9::ScreenUpdateProcess()
{
    ...
    else if (hr == D3DERR_DEVICENOTRESET)
    {
        ...
        ResetDevice();
        AVENGINE_WRN("ScreenUpdateProcess: ResetDevice");
    }
...
int VideoRenderDirect3D9::CloseDevice()
{
    ...
    AVENGINE_INF("CloseDevice: _pd3dDevice Release");
