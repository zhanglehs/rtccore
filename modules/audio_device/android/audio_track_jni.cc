...
AudioTrackJni::AudioTrackJni()
    : j_audio_track_(NULL),
      ...
      channels_(1),
      bytes_per_frame_(channels_*kBitsPerSample/8){
...
int32_t AudioTrackJni::InitPlayout() {
  ...
  jint delay_in_milliseconds = jni->CallIntMethod(
    j_audio_track_, initPlayoutID, sample_rate_hz_, channels_);
  CHECK_EXCEPTION(jni);
...
void AudioTrackJni::AttachAudioBuffer(AudioDeviceBuffer* audioBuffer) {
  ...
  audio_device_buffer_->SetPlayoutChannels(channels_);
}

int32_t AudioTrackJni::StereoPlayoutIsAvailable(bool& available) {
  //lfrtcGlobalConfig* config = GetGlobalConfig();
  //available = (config && (config->audio_render_channels == 2));
  available = true;
  return 0;
}
...
void AudioTrackJni::OnGetPlayoutData(int length) {
  ...
  DCHECK_EQ(frames_per_buffer_, length / bytes_per_frame_);
  ...
  DCHECK_EQ(length, bytes_per_frame_ * samples);
}
