...
int I420VideoFrame::CreateEmptyFrame(int width, int height,
  ...
  char *y = (char *)buffer(webrtc::kYPlane);
  char *u = (char *)buffer(webrtc::kUPlane);
  char *v = (char *)buffer(webrtc::kVPlane);
  memset(y, 0, width * height);
  memset(u, 128, half_width * (height >> 1));
  memset(v, 128, half_width * (height >> 1));
