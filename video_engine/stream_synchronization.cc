...
static const int kMaxDeltaDelayMs = 4000;
...
bool StreamSynchronization::ComputeDelays(int relative_delay_ms,
  ...
  if ((old_diff_ms_ >= 0 && avg_diff_ms_ > 0 && avg_diff_ms_ > old_diff_ms_)
    || (old_diff_ms_ <= 0 && avg_diff_ms_ < 0 && avg_diff_ms_ < old_diff_ms_)) {
    continue_adjust_count_++;
    diff_ms += avg_diff_ms_ * (continue_adjust_count_ - 1) / (continue_adjust_count_ + 1);
    diff_ms = std::min(diff_ms, 400);
    diff_ms = std::max(diff_ms, -400);
  }
  else {
    continue_adjust_count_ = 0;
  }
  ...
  avg_diff_ms_ -= diff_ms;
  if ((avg_diff_ms_ < 0 && diff_ms > 0) || (avg_diff_ms_ > 0 && diff_ms < 0)) {
    avg_diff_ms_ = 0;
  }

  //AVENGINE_INF("ComputeDelays:current_diff_ms=%d, old_diff_ms_=%d, avg_diff_ms_=%d, diff_ms=%d", current_diff_ms, old_diff_ms_, avg_diff_ms_, diff_ms);
  ...
  channel_delay_->extra_video_delay_ms = std::max(
      channel_delay_->extra_video_delay_ms, base_target_delay_ms_);
  channel_delay_->extra_video_delay_ms = std::min(
      channel_delay_->extra_video_delay_ms, base_target_delay_ms_ + kMaxDeltaDelayMs);
  channel_delay_->extra_audio_delay_ms = std::min(
      channel_delay_->extra_audio_delay_ms, base_target_delay_ms_ + kMaxDeltaDelayMs);
