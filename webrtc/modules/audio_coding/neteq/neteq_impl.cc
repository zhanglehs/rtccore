...
int NetEqImpl::InsertPacketInternal(const WebRtcRTPHeader& rtp_header,
  ...
  CodecInst receiveCodec = current_receive_codec_;
  payload_splitter_->SetAacCodecInst(receiveCodec);
...
int NetEqImpl::GetAudioInternal(size_t max_length, int16_t* output,
  ...
  *samples_per_channel = output_size_samples_;
  ...
  if (last_mode_ != kModeExpand) {
    ...
    else if (fs_hz_ > 0 && (static_cast<int32_t>(playout_timestamp_ - temp_timestamp) > fs_hz_ * 4)) {
      playout_timestamp_ = temp_timestamp;
      AVENGINE_WRN("GetAudioInternal, playout_timestamp_ error, reset");
    }
  } else {
