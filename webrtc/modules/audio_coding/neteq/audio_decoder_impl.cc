...
// Aac
//AudioDecoderAac::AudioDecoderAac(int num_channels) 
AudioDecoderAac::AudioDecoderAac(int num_channels, int chanId, void *cb, void *object, char *cparams)
: channels_(num_channels)
{
	DCHECK(num_channels == 1 || num_channels == 2);
	//WebRtcAac_DecoderCreate(&dec_state_, static_cast<int>(channels_));
	WebRtcAac_DecoderCreate(&dec_state_, static_cast<int>(channels_), chanId, cb, object, cparams);
}
...
int AudioDecoderAac::DecodeInternal(const uint8_t* encoded,
	...
	DCHECK_EQ(sample_rate_hz, 48000);
	int16_t temp_type = 1;  // Default is speech.
	int16_t ret = WebRtcAac_Decode(dec_state_, encoded,
		static_cast<int16_t>(encoded_len), decoded,
		&temp_type);
	if (ret > 0) {
		ret *= static_cast<int16_t>(channels_);  // Return total number of samples.
...
int AudioDecoderAac::DecodeRedundantInternal(const uint8_t* encoded,
	...
	DCHECK_EQ(sample_rate_hz, 48000);
	int16_t temp_type = 1;  // Default is speech.
	int16_t ret = WebRtcAac_DecodeFec(dec_state_, encoded,
		static_cast<int16_t>(encoded_len), decoded,
		&temp_type);
	if (ret > 0)
		ret *= static_cast<int16_t>(channels_);  // Return total number of samples.
...
int AudioDecoderAac::PacketDuration(const uint8_t* encoded,
	size_t encoded_len) const {
	return WebRtcAac_DurationEst(dec_state_,
		encoded, static_cast<int>(encoded_len));
}
...
int AudioDecoderAac::PacketDurationRedundant(const uint8_t* encoded,
  ...
  return WebRtcAac_FecDurationEst(dec_state_, encoded, static_cast<int>(encoded_len));
}

bool AudioDecoderAac::PacketHasFec(const uint8_t* encoded,
	size_t encoded_len) const {
	int fec;
	fec = WebRtcAac_PacketHasFec(encoded, static_cast<int>(encoded_len));
	return (fec == 1);
}
...
AudioDecoder* CreateAudioDecoder(NetEqDecoder codec_type, CodecInst &receive_codec) {
...
#endif
	case kDecoderAac1: {
      int chanId = receive_codec.chanId;
      void *cb = receive_codec.cb;
      void *object = receive_codec.object;
      char *cparams = receive_codec.params;
      return new AudioDecoderAac(2, chanId, cb, object, cparams);
		}
    case kDecoderCNGnb:
    case kDecoderCNGwb:
    case kDecoderCNGswb32kHz:
