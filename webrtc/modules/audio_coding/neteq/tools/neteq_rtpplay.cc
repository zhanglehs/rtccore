...
DEFINE_int32(aac0, 114, "RTP payload type for Aac0");
const bool aac0_dummy =
google::RegisterFlagValidator(&FLAGS_aac0, &ValidatePayloadType);
DEFINE_int32(aac1, 115, "RTP payload type for Aac1");
const bool aac1_dummy =
google::RegisterFlagValidator(&FLAGS_aac1, &ValidatePayloadType);
...
std::string CodecName(webrtc::NetEqDecoder codec) {
  switch (codec) {
    ...
    case webrtc::kDecoderOpus:
      return "Opus";
	case webrtc::kDecoderAac0:
		return "Aac0";
	case webrtc::kDecoderAac1:
		return "Aac1";
...
void RegisterPayloadTypes(NetEq* neteq) {
  ...
  RegisterPayloadType(neteq, webrtc::kDecoderAac0, FLAGS_aac0);
  RegisterPayloadType(neteq, webrtc::kDecoderAac1, FLAGS_aac1);
...
void PrintCodecMapping() {
  ...
  PrintCodecMappingEntry(webrtc::kDecoderAac0, FLAGS_aac0);
  PrintCodecMappingEntry(webrtc::kDecoderAac1, FLAGS_aac1);
...
int CodecSampleRate(uint8_t payload_type) {
  ...
  if (payload_type == FLAGS_opus || payload_type == FLAGS_pcm16b_swb48 ||
    ...
    return 48000;
  if (payload_type == FLAGS_aac0 || payload_type == FLAGS_aac1)
	  return 48000;
