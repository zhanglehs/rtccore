...
void ACMGenericCodec::ResetAudioEncoder() {
  ...
  if (!STR_CASE_CMP(codec_inst.plname, "PCMU")) {
    ...
    audio_encoder_.reset(new AudioEncoderOpus(config));
	  ...
	  is_aac_ = true;
	  has_internal_fec_ = false;// true;
	  AudioEncoderAac::Config config;
	  strcpy(config.params, codec_inst.params);
	  config.frame_size_ms = codec_inst.pacsize / 48;
	  config.num_channels = codec_inst.channels;
	  config.fec_enabled = fec_enabled_;
	  config.bitrate_bps = codec_inst.rate;
	  config.max_playback_rate_hz = max_playback_rate_hz_;
	  config.dtx_enabled = false;// opus_dtx_enabled_;
	  config.payload_type = codec_inst.pltype;
    config.cb = codec_inst.cb;
    config.object = codec_inst.object;
    config.channelid = codec_inst.chanId;
	  switch (GetOpusApplication(config.num_channels, config.dtx_enabled)) {
	  case kVoip:
		  config.application = AudioEncoderAac::ApplicationMode::kVoip;
		  break;
	  case kAudio:
		  config.application = AudioEncoderAac::ApplicationMode::kAudio;
		  break;
	  }
	  audio_encoder_.reset(new AudioEncoderAac(config));
