...
void SendReport(void *userdata,
  ...
  static int64_t t = 0;
  int64_t now = webrtc::TickTime::MillisecondTimestamp();
  if (now - t > 5000) {
    t = now;

    Json::Value root;
    Json::Value value;
    value["decode_ms"] = decode_ms;
    value["max_decode_ms"] = max_decode_ms;
    value["current_delay_ms"] = current_delay_ms;
    value["target_delay_ms"] = target_delay_ms;
    value["jitter_buffer_ms"] = jitter_buffer_ms;
    root["videojitter"] = value;
    Json::FastWriter writer;
    SendAvengineReport(userdata, writer.write(root).c_str());
  }
}
}
