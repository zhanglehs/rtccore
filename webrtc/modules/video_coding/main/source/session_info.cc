...
int VCMSessionInfo::InsertPacket(const VCMPacket& packet,
  ...
  if (packet.isFirstPacket) {
    ...
    for (auto it = packets_.begin(); it != packets_.end(); it++) {
      assert(IsNewerSequenceNumber(it->seqNum, packet.seqNum));
    }
  }
  else if (packet.markerBit) {
    assert(last_packet_seq_num_ == -1);
    for (auto it = packets_.begin(); it != packets_.end(); it++) {
      assert(IsNewerSequenceNumber(packet.seqNum, it->seqNum));
    }
  }
  else {
    if (first_packet_seq_num_ != -1) {
      assert(IsNewerSequenceNumber(packet.seqNum, first_packet_seq_num_));
    }
  ...
  if (packet.codec == kVideoCodecUnknown || packet.codec == kVideoCodecH264) {
