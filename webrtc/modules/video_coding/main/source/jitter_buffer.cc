...
void FrameList::CleanUpOldOrEmptyFrames(VCMDecodingState* decoding_state,
  ...
  while (!empty()) {
    ...
    const std::list<VCMPacket>& packets = oldest_frame->GetPacketList();
    for (auto it = packets.begin(); it != packets.end(); it++) {
      AVENGINE_TRC("JitterBuffer_DropPacket: timestamp=%u, seqNum=%d, last_decode_seqnum=%d, reason=old frame", it->timestamp, (int)it->seqNum, (int)decoding_state->sequence_num());
    }
...
void VCMJitterBuffer::Flush(const char *reason) {
  CriticalSectionScoped cs(crit_sect_);
  if (reason == NULL) {
    reason = "flush";
  }

  for (auto i = decodable_frames_.begin(); i != decodable_frames_.end(); i++) {
    const std::list<VCMPacket> &packets = i->second->GetPacketList();
    for (auto j = packets.begin(); j != packets.end(); j++) {
      AVENGINE_TRC("JitterBuffer_DropPacket: timestamp=%u, seqNum=%d, last_decode_seqnum=%d, reason=%s", j->timestamp, (int)j->seqNum, (int)last_decoded_state_.sequence_num(), reason);
    }
  }
  for (auto i = incomplete_frames_.begin(); i != incomplete_frames_.end(); i++) {
    const std::list<VCMPacket> &packets = i->second->GetPacketList();
    for (auto j = packets.begin(); j != packets.end(); j++) {
      AVENGINE_TRC("JitterBuffer_DropPacket: timestamp=%u, seqNum=%d, last_decode_seqnum=%d, reason=%s", j->timestamp, (int)j->seqNum, (int)last_decoded_state_.sequence_num(), reason);
    }
...
VCMEncodedFrame* VCMJitterBuffer::ExtractAndSetDecode(uint32_t timestamp) {
  ...
  */
  ...
  if ((*frame).IsSessionComplete()) {
    ...
    const std::list<VCMPacket>& packets = frame->GetPacketList();
    for (auto it = packets.begin(); it != packets.end(); it++) {
      AVENGINE_TRC("JitterBuffer_ExtractIncompleteFrame: timestamp=%u, seqNum=%d, last_decode_seqnum=%d", it->timestamp, (int)it->seqNum, (int)last_decoded_state_.sequence_num());
    }
  }
...
VCMFrameBufferEnum VCMJitterBuffer::InsertPacket(const VCMPacket& packet,
  ...
  if (last_decoded_state_.IsOldPacket(&packet)) {
    ...
    AVENGINE_TRC("JitterBuffer_DropPacket: timestamp=%u, seqNum=%d, last_decode_seqnum=%d, reason=old packet", packet.timestamp, (int)packet.seqNum, (int)last_decoded_state_.sequence_num());
    ...
    if (num_consecutive_old_packets_ > kMaxConsecutiveOldPackets) {
      ...
      Flush("too many old packets flush");
      return kFlushIndicator;
    }
    return kOldPacket;
  }
  ...
  case kDuplicatePacket:
    AVENGINE_TRC("JitterBuffer_DropPacket: timestamp=%u, seqNum=%d, last_decode_seqnum=%d, reason=duplicate packet", packet.timestamp, (int)packet.seqNum, (int)last_decoded_state_.sequence_num());
  case kNoError:
