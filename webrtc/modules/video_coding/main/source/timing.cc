...
int64_t VCMTiming::RenderTimeMsInternal(uint32_t frame_timestamp,
  ...
  static int64_t s_log_time = 0;
  if (s_log_time / 1000 != now_ms / 1000) {
    s_log_time = now_ms;
    AVENGINE_TRC(
      "video jitter, jitter_delay_ms=%u, max_decode_ms=%d, render_delay_ms=%u, current_delay_ms=%u, min_playout_delay_ms_=%u",
      jitter_delay_ms_, MaxDecodeTimeMs(), render_delay_ms_, current_delay_ms_,
