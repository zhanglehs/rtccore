...
bool VideoRenderIosGles20::ScreenUpdateProcess() {
  ...
  std::set<VideoRenderIosChannel*> first_channels;

  // Check if there are any updated buffers
  bool updated = false;

  for (auto it = agl_channels_.begin(); it != agl_channels_.end(); it++) {
    VideoRenderIosChannel* agl_channel = it->second;
    bool channel_updated = false;
    bool first_updated = false;
    agl_channel->IsUpdated(channel_updated, first_updated);

    if (channel_updated) {
      updated = true;
    }
    if (first_updated) {
      first_channels.insert(agl_channel);
    }
  }
