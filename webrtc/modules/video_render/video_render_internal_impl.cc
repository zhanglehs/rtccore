...
void *GetRenderWindowId(void *window) {
#ifdef WEBRTC_ANDROID
  bool isAttached = false;
  JavaVM* jvm = VideoRenderAndroid::g_jvm;
  JNIEnv* env = NULL;
  if (jvm->GetEnv((void**)&env, JNI_VERSION_1_4) != JNI_OK) {
    ...
    if ((res < 0) || !env) {
      AVENGINE_ERR("Could not attach thread from JVM (%d, %p)", res, env);
      return NULL;
    }
    isAttached = true;
  }

  jobject obj = (jobject)window;
  jclass clazz = env->GetObjectClass(obj);
  jmethodID mid = env->GetMethodID(clazz, "hashCode", "()I");
  int id = env->CallIntMethod(obj, mid);
  env->DeleteLocalRef(clazz);
  if (isAttached) {
    if (jvm->DetachCurrentThread() < 0) {
      AVENGINE_ERR("Could not detach thread from JVM");
    }
  }
  return (void *)id;
#else
...
{
    _windowId = GetRenderWindowId(window);
