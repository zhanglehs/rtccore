...
void IncomingVideoStream::SetUserdata(void* data) {
  if (render_callback_) {
    render_callback_->SetUserdata(data);
  }
}
...
bool IncomingVideoStream::IncomingVideoStreamProcess() {
  if (kEventError != deliver_buffer_event_.Wait(KEventMaxWaitTimeMs)) {
    ...
    if (frame_to_render.IsZeroSize()) {
      if (render_callback_) {
        if (last_render_time_ms_ == 0 && !start_image_.IsZeroSize()) {
          ...
          last_frame_.CopyFrame(start_image_);
        ...
        else if (!last_frame_.IsZeroSize()) {
          temp_frame_.CopyFrame(last_frame_);
          render_callback_->RenderFrame(stream_id_, temp_frame_);
        }
      }
    ...
    last_frame_.CopyFrame(frame_to_render);
