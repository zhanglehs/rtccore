...
class WebRtcAudioTrack {
  ...
  private int InitPlayout(int sampleRate, int channels) {
    Logd("InitPlayout(sampleRate=" + sampleRate + ", channels=" + channels + ")");
    ...
    if (minBufferSizeInBytes < byteBuffer.capacity()) {
      Loge("AudioTrack.getMinBufferSize returns an invalid value.");
      return -1;
    }
    ...
    Logd("AudioTrack.streamtype: " + audioTrack.getStreamType());
  ...
  private boolean StartPlayout() {
    ...
    if (audioTrack.getState() != AudioTrack.STATE_INITIALIZED) {
      Loge("AudioTrack instance is not successfully initialized.");
      return false;
    }
