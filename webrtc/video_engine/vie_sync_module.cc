...
ViESyncModule::ViESyncModule(VideoCodingModule* vcm,
    ...
    : data_cs_(CriticalSectionWrapper::CreateCriticalSection()),
      ...
      sync_(),
      relative_delay_ms_(0),
      target_audio_delay_ms_(0),
      target_video_delay_ms_(0) {
}
...
int32_t ViESyncModule::Process() {
  ...
  if (UpdateMeasurements(&video_measurement_, *video_rtp_rtcp_,
                         *video_receiver_) != 0) {
    AVENGINE_WRN("Err_Sync:UpdateMeasurements_v = %d", 0);
    relative_delay_ms_ = 0;
    return 0;
  }
  ...
  if (UpdateMeasurements(&audio_measurement_, *voice_rtp_rtcp,
                         *voice_receiver) != 0) {
    relative_delay_ms_ = 0;
    AVENGINE_WRN("Err_Sync:UpdateMeasurements_a = %d", 0);
    return 0;
  }

  int relative_delay_ms = 0;
  // Calculate how much later or earlier the audio stream is compared to video.
  if (!sync_->ComputeRelativeDelay(audio_measurement_, video_measurement_,
                                   &relative_delay_ms)) {
    relative_delay_ms_ = 0;
    AVENGINE_WRN("Err_Sync:ComputeRelativeDelay = %d", 0);
    return 0;
  }
  relative_delay_ms_ = (relative_delay_ms + relative_delay_ms_) / 2;
  relative_delay_ms = relative_delay_ms_;
  ...
  AVENGINE_TRC("Current Delay:current_delay_ms = %d", current_video_delay_ms);
  AVENGINE_TRC("vie_sync: net_diff=%d, current_audio=%d(%d+%d), current_video=%d, diff=%d",
    relative_delay_ms, current_audio_delay_ms, audio_jitter_buffer_delay_ms,
    playout_buffer_delay_ms, current_video_delay_ms,
  ...
  bool changed = sync_->ComputeDelays(relative_delay_ms,
                            current_audio_delay_ms,
                            &target_audio_delay_ms,
                            &target_video_delay_ms);
  if (changed) {
    target_audio_delay_ms_ = target_audio_delay_ms;
    target_video_delay_ms_ = target_video_delay_ms;
  }
  Json::Value value;
  value["current_video_delay"] = current_video_delay_ms;
  value["current_audio_delay"] = current_audio_delay_ms;
  value["delay_diff"] = current_video_delay_ms - current_audio_delay_ms + relative_delay_ms;
  value["target_video_delay"] = target_video_delay_ms_;
  value["target_audio_delay"] = target_audio_delay_ms_;
  Json::Value root;
  root["sync"] = value;
  Json::FastWriter writer;
  SendAvengineReport(vie_channel_->GetUserdata(), writer.write(root).c_str());
  if (!changed) {
    return 0;
  }
  ...
  AVENGINE_TRC("vie_sync: target_audio=%d, target_video=%d",
